...
結果について：
 “pickup は、買える銘柄の 候補 に過ぎず「買って良い」ではなく「検討に値する」銘柄です。 
 次は 時間軸とチャート状態 を重ねて、最終候補（entry-ready） を選抜する段階となります。
...
入力CSVの推奨列:
    ...
    win_prob,rr_ratio,win_amount,risk_span,max_dd_ratio,comment

- win_prob, rr_ratio, win_amount, risk_span, max_dd_ratio は省略可
...
--input (-i) 文字列（ファイル名） candidate_tri5.csv 入力CSVファイル
--default-win-prob 浮動小数 (0〜1) :0.35 勝率（全体）
--jp-adjust 浮動小数 0.05 JP銘柄への勝率補正
--rr-ratio 浮動小数 (>0) 2.0 損益比率（R/R）
--base-dir 文字列（ディレクトリ） res_qg_selection 結果保存のベースdir
--suffix (-z) 文字列 "" 出力dir末尾コメント
--config-only フラグ False configのみ生成
--tri5-threshold 浮動小数 0.0 TRI5でPICK判定する閾値

--risk-span-mul-fast 浮動小数 (>0) 2.0 期間 <=1ヶ月用 係数
--risk-span-mul-swing 浮動小数 (>0) 1.5 1〜4ヶ月用 係数
--risk-span-mul-core 浮動小数 (>0) 1.2 4ヶ月超用 係数
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="銘柄候補の期待値・TRI3/TRI5/TRI6 を計算するツール"
    )
    ...
    # risk_span 自動推定用係数 (期間ごとの倍率)
    parser.add_argument(
        "--risk-span-mul-fast",
        type=float,
        default=2.0,
        help="period_months <= 1 のときの risk_span 推定倍率 (loss_amount × この値)",
    )
    parser.add_argument(
        "--risk-span-mul-swing",
        type=float,
        default=1.5,
        help="1 < period_months <= 4 のときの risk_span 推定倍率",
    )
    parser.add_argument(
        "--risk-span-mul-core",
        type=float,
        default=1.2,
        help="period_months > 4 のときの risk_span 推定倍率",
    )
...
def _clamp_ratio(x: float, lo: float = 0.0, hi: float = 1.0) -> float:
    return max(lo, min(hi, x))
...
def calc_metrics(
    *,
    ...
    """
    1銘柄分の各種指標を計算する。
    risk_span は「この戦略1回あたりで想定される典型的な損益変動幅（円）」。
    """
...
def main() -> None:
    ...
    config_data: Dict[str, Any] = {
        ...
        "risk_span_mul_fast": args.risk_span_mul_fast,
        "risk_span_mul_swing": args.risk_span_mul_swing,
        "risk_span_mul_core": args.risk_span_mul_core,
        ...
        "note": (
            ...
            "risk_span は CSV の risk_span 列が優先。空欄なら loss_amount と period_months から自動推定。"
            "max_dd_ratio (0〜1) があれば TRI6_score を TRI5_score にドローダウン補正を掛けて算出。"
        ),
        "required_columns": [
            "ticker",
            "market",
    ...
    with input_path.open("r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            risk_span_override = None
            if "risk_span" in row and row["risk_span"].strip() != "":
                try:
                    risk_span_override = float(row["risk_span"].strip())
                except ValueError:
                    risk_span_override = None

            max_dd_ratio = None
            if "max_dd_ratio" in row and row["max_dd_ratio"].strip() != "":
                try:
                    max_dd_ratio = _clamp_ratio(
                        float(row["max_dd_ratio"].strip()), 0.0, 1.0
                    )
                except ValueError:
                    max_dd_ratio = None

            # risk_span の最終値を決定
            if risk_span_override is not None and risk_span_override > 0:
                risk_span_effective = risk_span_override
            else:
                # 自動推定ルール
                if period_months <= 1.0:
                    mul = args.risk_span_mul_fast
                elif period_months <= 4.0:
                    mul = args.risk_span_mul_swing
                else:
                    mul = args.risk_span_mul_core
                risk_span_effective = loss_amount * mul
            ...
            # risk_span_effective も出力（自動推定を含む最終値）
            out_row["risk_span_effective"] = f"{risk_span_effective:.2f}"
            ...
            # TRI6 (最大DD補正付きスコア)
            tri6_val: Optional[float] = None
            if tri5_val is not None and max_dd_ratio is not None:
                dd_factor = max(0.1, 1.0 - max_dd_ratio)
                tri6_val = tri5_val * dd_factor
                out_row["TRI6_score"] = f"{tri6_val:.2f}"
                out_row["dd_factor"] = f"{dd_factor:.3f}"
                out_row["max_dd_ratio_effective"] = f"{max_dd_ratio:.3f}"
            else:
                # max_dd_ratio がない場合は TRI6 は空欄
                out_row["TRI6_score"] = ""
                out_row["dd_factor"] = ""
                out_row["max_dd_ratio_effective"] = (
                    f"{max_dd_ratio:.3f}" if max_dd_ratio is not None else ""
                )

            rows_out.append(out_row)

    # ソート: TRI6 優先 (なければ TRI5、それもなければ TRI3、それでもなければ 0)
    def sort_key(r: Dict[str, Any]) -> float:
        for key in ("TRI6_score", "TRI5_score", "TRI3_yen_per_min_per_year"):
            v = r.get(key, "")
            try:
                return float(v)
            except ValueError:
    ...
    if rows_out_sorted:
        ...
        # ticker の右隣に TRI6_score, TRI5_score を配置
        preferred_front = ["ticker", "TRI6_score", "TRI5_score"]
